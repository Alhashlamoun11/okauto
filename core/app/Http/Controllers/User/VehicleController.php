...
class VehicleController extends Controller
{
    public function store()
    {
        ...
        return view('Template::user.vehicle.store', compact('pageTitle', 'form', 'user', 'zones'));
    }
    ...
    public function storeData()
    {
        ...
        $user      = auth()->user();
        return view('Template::user.vehicle.store_data', compact('pageTitle', 'user'));
    }
    ...
    public function index()
    {
        ...
        return view('Template::user.vehicle.index', compact('pageTitle', 'vehicles'));
    }
    ...
    public function add($id = 0)
    {
        ...
        if ($id) {
            ...
            $vehicle   = Vehicle::with('extraServices.vehicleExtraServices')->where('user_id', auth()->id())->findOrFail($id);
        ...
        $user   = auth()->user();
        $zones  = Zone::active()->get(['id', 'name']);
        $extraServices = ExtraService::where('active', 1)->get();

        return view('Template::user.vehicle.add', compact('extraServices','pageTitle', 'vehicle', 'brands', 'images', 'zones'));
    }
    ...
    public function update(Request $request, $id = 0)
    {
        ...
        $validate  = [
            ...
            'day_hours'             => 'required|numeric|gte:0',
...
//             'total_run'             => 'required|numeric|gte:0',
    ...
    private function insertVehicleData($vehicle, $request, $images)
    {
        ...
        $vehicle->day_hours             = $request->day_hours??12;
        ...
        $vehicle->security_deposit      = $request->security_deposit;
        ...
        if ($request->has('extra_services')) {
            foreach ($request->extra_services as $id => $data) {
                if(isset($data['id']))
                $syncData[$id] = [
                    'custom_price' => $data['price'] ?? 0,
                ];
            }
        }
        $vehicle->extraServices()->sync($syncData);
    ...
    public function detail($id)
    {
        ...
        return view('Template::user.vehicle.detail', compact('pageTitle', 'vehicle'));
    }

    public function storeUpdateSettings(Request $request){
        auth()->user()->cancelation_hours=$request->cancelation_hours;
        auth()->user()->allowed_age=$request->allowed_age;
        auth()->user()->save();
                    $notification = 'Data Updated successfulle';

                $notify[] = ['success', $notification];
        return back()->withNotify($notify);

        // dd(auth()->user()->cancelation_hours);
