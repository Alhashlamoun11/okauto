...
class ManageVehicleTypeController extends Controller {
    ...
    public function typeStore(Request $request, $id = 0) {
        ...
        $request->validate([
            ...
            'slug'        => 'unique:vehicle_types,slug,' . $id,
        ]);
        ...
        if ($id) {
            ...
            $vehicle->slug = slug($request->name) . '-' .time();
        ...
        $vehicle->name_keywords         = $request->keywords;
        $vehicle->head         = $request->head;
        $vehicle->title         = $request->title;
        $vehicle->description  = $request->description;
        $vehicle->meta_description  = $request->meta_description;
    ...
    public function classStore(Request $request, $id = 0) {
        $request->validate([
            ...
            'slug' => 'unique:vehicle_classes,slug,' . $id,
        ], [
            'name.unique' => 'The name must be unique within the selected vehicle type',
            'slug.unique' => 'The slug must be unique within the selected vehicle type',
        ]);
