...
//     }
    ...
    public function Deposit(){
        return $this->hasOne(Deposit::class,'rent_id');
    }
    ...
    public function badgeData() {
        ...
        if ($this->status == Status::RENT_INITIATE) {
            ...
            $html = '<span style="background:#32fd27!important;color:white!important;" class="badge badge--primary">' . trans('Approved') . '</span>';
    ...
    public function getPriceAttribute($value){
        ...
        if($coupon && $coupon->type==0){
            $final_price=(float) $final_price-($final_price*($coupon->value/100));
        }
        
        $deposit=Deposit::where('rent_id',$this->id)->first();
        if($deposit){
    ...
    public function extraServices()
    {
        return $this->hasMany(RentalExtraService::class);
    }
