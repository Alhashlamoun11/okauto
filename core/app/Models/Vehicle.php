...
class Vehicle extends Model
{
    ...
    public function getDayHours($value){
        return $value;
    }
    ...
    public function scopeAvailable($query)
    {
        $query->active()->approved()->whereHas('user', function ($query) {
            ...
            $q->where("vehicle_class_id", 0)->orWhereHas('vehicleClass', function ($query) {
                $query->active();
            });
        });
    }
    ...
    {
        return $this->belongsToMany(ExtraService::class, 'vehicle_extra_services')
