...
<div class="search-form">
    <div class="container">
        <form action="{{ route('vehicles') }}" method="GET">
            <div style="padding:16px" class="search-form__wrapper">
                <div class="search-form-item position-relative">
                    <label class="form--label">{{__('Pick Up')}}</label>
                    <select class="form--control  select2-basic hidden-select2" name="pick_up_zone_id" required>
                ...
                <div class="search-form-item position-relative">
                    ...
                    <select class="form--control  select2-basic hidden-select2" name="vehicle_type_id">
                        <option value="" selected disabled>@lang('Select One')</option>
                ...
                {{--<div class="search-form-item position-relative">
                    ...
                    <select class="form--control select2-basic hidden-select2" name="drop_off_zone_id" required>
                        <option value="" selected disabled>@lang('Select One')</option>
                        @foreach ($zones as $zone)
                            <option value="{{ $zone->id }}">{{ __($zone->name) }}</option>
                        @endforeach
                ...
                <div class="search-form-item">
                    ...
                    <input name="pickup_time" id="pickup_time"
                           type="time" data-language="en" class="date-filter form--control form-datepicker"  />
                </div>
                <div class="search-form-item">
                    <label class="form-label">@lang('Drop Off Time')</label>
                    <input name="dropoff_time" id="dropoff_time"
                           type="time" data-language="en" class="date-filter form--control form-datepicker"/>
                </div>
