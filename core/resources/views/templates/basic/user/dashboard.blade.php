...
@section('content')
    ...
    @if ($user->store == Status::STORE_INITIATE || $user->store == Status::STORE_REJECTED)
        <div class="alert alert-danger mb-4" role="alert">
            <h6 class="mb-2 text--danger">@lang('Register Vehicle Store')</h6>
            <p class="mb-0 text-dark">{{ __(@$vehicleContent->data_values->store_required_content) }}<a class="text--danger ms-1" href="{{ route('user.vehicle.store') }}">@lang('Click Here to Start')</a></p>
        </div>
...
<!--                     <span class="dashboard-widget__text">@lang('Total Balance')</span> -->
					@if($user->role == 'vendor')
    					{{-- Vendor: Show total_balance (calculated from income - withdrawals) --}}
    					<h4 class="dashboard-widget__number">{{ showAmount($widget['total_balance']) }}</h4>
					@else
    					{{-- Renter: Show actual wallet balance --}}
    					<h4 class="dashboard-widget__number">{{ showAmount($widget['total_balance']) }}</h4>
					@endif
						<span class="dashboard-widget__text">@lang('Total Balance')</span>
            ...
            <div class="col-md-6 col-sm-6 col-xsm-6">
                <div class="dashboard-widget">
                    ...
                    <h4 class="dashboard-widget__number">{{ showAmount($widget['total_income']) }}
                    </h4>
            ...
            <div class="col-md-6 col-sm-6 col-xsm-6">
                <div class="dashboard-widget">
                    ...
                    <h4 class="dashboard-widget__number">
                        {{ showAmount($widget['total_withdrawan']) }}</h4>
                    <span class="dashboard-widget__text">@lang('Total Withdrawn')</span>
                </div>
            </div>
            @endif
        </div>
...
@push('script')
    <script>
        (function($) {
            ...
            {{--    var dateFromSession = "{{ old('date', session('booking_details.date')) }}";--}}
            {{--    if (dateFromSession) {--}}
            {{--        $('#date').datepicker().data('datepicker').selectDate(new Date(dateFromSession));--}}
            {{--    }--}}
            {{--});--}}
        })(jQuery)
    </script>
