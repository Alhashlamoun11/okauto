...
@section('panel')
<button class="btn btn-sm btn-outline--dark" onclick="printSection()">Print</button>
@if($rental->status!=Status::RENT_CANCELLED && $rental->status!=Status::RENT_INITIATE)
<a href="{{ route('user.reservation.invoice', $rental->id) }}" class="btn btn--info btn-sm" target="_blank">
    <i class="fas fa-file-invoice"></i> @lang('View Invoice')
</a>

<a href="{{ route('user.reservation.invoice.download', $rental->id) }}" class="btn btn--dark btn-sm">
    <i class="fas fa-download"></i> @lang('Download Invoice')
</a>
@endif

    <div id="rental_detail_section" class="row gy-3">
        <div class="col-xxl-4 col-xl-5 col-lg-12">
            ...
            <div class="card user-card mt-4 b-radius--10 overflow-hidden box--shadow1">
                ...
                <div class="card-body">
                    <div class="user__profile">
                        <div class="user__profile-thumb">
                            <img src="{{ getImage(getFilePath('userProfile') . '/' . @$rental->vehicleOwner->image, getFileSize('userProfile')) }}" alt="@lang('Image')">
                        </div>
                        <div class="user__profile-info">
                        ...
                        <div class="row gy-4 align-items-center">
                            <div class="col-md-6">
                                ...
                                <div class="product-info-bottom">
                                    ...
                                    <h6 class="product-info-bottom-price">{{ showAmount(@$rental->price) }}</h6>
                                </div>
                            </div>
                            <div class="col-md-6">
            ...
            <div class="card b-radius--10 overflow-hidden box--shadow1 mt-4">
                <div class="card-body">
                    ...
                    <ul class="list-group list-group-flush">
                        ...
                        <li class="list-group-item d-flex justify-content-between align-items-center px-0">
                            @lang('PickUp Time')
                            <span class="fw-bold">{{ @$rental->pickup_time??"00:00" }}</span>
                        </li>
                        <li class="list-group-item d-flex justify-content-between align-items-center px-0">
                            @lang('End Date')
                        ...
                        <li class="list-group-item d-flex justify-content-between align-items-center px-0">
                            @lang('DropOff Time')
                            <span class="fw-bold">{{ @$rental->dropoff_time??"00:00" }}</span>
                        </li>
                        ...
                        <div class="card b-radius--10 overflow-hidden box--shadow1 mt-4">
                <div class="card-body">
                    <h5 class="mb-20 text-muted">@lang('Extra Services')</h5>
                    <ul class="list-group list-group-flush">
                    @foreach($rental->extraServices as $extraService)
                    	<li>{{$extraService->extraService->name}} - {{$extraService->price}} {{__(gs('cur_text'))}}</li>
                    @endforeach
          
                    </ul>
                </div>
            </div>
                      @if($rental->status==Status::RENT_CANCELLED)
                        <div class="card b-radius--10 overflow-hidden box--shadow1 mt-4">
                <div class="card-body">
                    <h5 class="mb-20 text-muted">@lang('Cancellation Reason')</h5>
                    <ul class="list-group list-group-flush">
                            <p class="fw-bold">{!! preg_replace('/,\s*/', "<br>", @$rental->cancellation_reason??"") !!}</p>
...
@push('breadcrumb-plugins')
    <x-back route="{{ route('admin.rental.index') }}" />
@endpush
...
function printSection() {
    const section = document.getElementById("rental_detail_section");
