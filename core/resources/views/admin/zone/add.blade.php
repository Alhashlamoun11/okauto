                ...
                <div class="card-body">
                    <form action="{{ route('admin.zone.store', @$zone->id) }}" method="POST" id="zoneForm">
                        @csrf
                            ...
                            <input type="hidden" name="zoom" id="zoom" value="{{ old('zoom', @$zone->zoom ?? '14') }}">
                        ...
                        <div class="form-group">
                            <label>@lang('Mark Your Zone')</label>
                            <input id="pac-input" class="controls" type="text" placeholder="Search Box" @if(!isset($zone->id)) required @endif>
        ...
        function InitMap() {
            ...
            searchBox.addListener('places_changed', function() {
                ...
                places.forEach(function(place) {
                    if (!place.geometry) {
                        notify("No geometry found", "error");
                        return;
                    }
                    var icon = {
                        url: place.icon,
