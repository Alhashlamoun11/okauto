...
<div class="sidebar bg--dark">
    ...
    <div class="sidebar__inner">
        <div class="sidebar__logo">
            <a href="{{route('admin.dashboard')}}" class="sidebar__main-logo"><img src="{{siteLogo('white')}}" alt="image"></a>
        </div>
        <div class="sidebar__menu-wrapper">
            <ul class="sidebar__menu">
                @foreach($sideBarLinks as $key => $data)
                
				@if($isAdmin || (!isset($data->access_code) || in_array($data->access_code ?? 0, json_decode($access ?? '[]', true)))) 
                   @if (@$data->header)
        ...
        <div class="version-info text-center text-uppercase">
            ...
            <span class="text--primary">{{__("OkAuto")}}</span>
        </div>
    </div>
</div>
<!-- sidebar end -->
